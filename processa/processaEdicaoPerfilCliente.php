...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === 0) {
        ...
        if (move_uploaded_file($foto['tmp_name'], $uploadPath)) {
            ...
            if (!empty($usuarioAtual['foto_perfil']) && $usuarioAtual['foto_perfil'] !== 'avatar_default.png') {
                $oldPath = $uploadDir . $usuarioAtual['foto_perfil'];
    ...
    if ($sucesso) {
        ...
        header('Location: ../painel/gerenciarPerfilCliente.php?sucesso=edicao');
        exit;
    } else {
        header('Location: ../painel/gerenciarPerfilCliente.php?erro=edicao');
        exit;
    }
